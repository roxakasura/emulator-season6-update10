...
CServerDisplayer::CServerDisplayer() // OK
{
	...
	this->m_brush[2] = CreateSolidBrush(RGB(1,7,13));
...
void CServerDisplayer::SetWindowName() // OK
{
    ...
    wsprintf(buff,"[%s] %s JoinServer (QueueSize : %d) (AccountCount : %d/%d)",JOINSERVER_VERSION,JOINSERVER_CLIENT1,gSocketManager.GetQueueSize(),
