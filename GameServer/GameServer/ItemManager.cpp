...
int CItemManager::RepairItem(LPOBJ lpObj,CItem* lpItem,int slot,int type) // OK
{
	...
	return lpObj->Money;
   //=====================================================================
   //  Fix Bug de Zen (Con interfaces Abiertas) - Su costo Disminuia de (Ejm: 5200 a 2000)
   //  LINK: tuservermu.com.ve/index.php?topic=30978.0
   //======================================================================================================================
   if (lpObj->Interface.type == INTERFACE_TRADE || lpObj->Interface.type == INTERFACE_WAREHOUSE || lpObj->Interface.type == INTERFACE_SHOP)
   {
      gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"Não pode reparar item com essa interface aberta");
      return 0;
   }
...
void CItemManager::CGItemGetRecv(PMSG_ITEM_GET_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		PMSG_MUUN_ITEM_GET_SEND plMsg;

		plMsg.header.setE(0x4E, 0x00, sizeof(plMsg));

		plMsg.result = 0xFF;

		memset(plMsg.ItemInfo, 0, sizeof(plMsg.ItemInfo));

		if ((plMsg.result = gMuunSystem.MuunInventoryInsertItem(aIndex, item)) == 0xFF)
		{
			DataSend(aIndex, (BYTE*)&plMsg, plMsg.header.size);
			return;
		}

		gMap[lpObj->Map].ItemGive(aIndex, index);

		this->ItemByteConvert(plMsg.ItemInfo, item);
...
void CItemManager::CGItemDropRecv(PMSG_ITEM_DROP_RECV* lpMsg,int aIndex) // OK
{
	...
	else if(lpItem->IsMuunItem())
	{
		GDCreateItemSend(lpObj->Index,0xED,0,0,lpItem->m_Index,lpItem->m_Level,(BYTE)lpItem->m_Durability,lpItem->m_Option1,lpItem->m_Option2,lpItem->m_Option3,lpObj->Index,lpItem->m_NewOption,lpItem->m_SetOption,lpItem->m_JewelOfHarmonyOption, lpItem->m_ItemOptionEx, lpItem->m_SocketOption,lpItem->m_SocketOptionBonus,lpItem->m_PeriodicItemTime);
		this->InventoryDelItem(aIndex,lpMsg->slot);
	}
...
void CItemManager::CGItemMoveRecv(PMSG_ITEM_MOVE_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpMsg->SourceFlag == 22 && lpMsg->TargetFlag == 4) // Muun Inventory -> Personal Shop
	{
		if((pMsg.result=this->MoveItemToPersonalShopFromMuunInventory(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot, lpMsg->TargetFlag)) != 0xFF)
		{
			this->ItemByteConvert(pMsg.ItemInfo, lpObj->Inventory[lpMsg->TargetSlot]);
		}
	}
	if(lpMsg->SourceFlag == 4 && lpMsg->TargetFlag == 22) // Personal Shop -> Muun Inventory
	{
		if((pMsg.result=this->MoveItemToMuunInventoryFromPersonalShop(lpObj,lpMsg->SourceSlot,lpMsg->TargetSlot,lpMsg->TargetFlag)) != 0xFF)
		{
			this->ItemByteConvert(pMsg.ItemInfo,lpObj->MuunInventory[lpMsg->TargetSlot]);
		}
	}
...
//FIX  MuEMU CashShop & Chaos Machine DUP
...
// Right Click direto - equipar ou mover itens pelo click
...
void CItemManager::GCItemListViewSend(PMSG_VIEW_REQUEST_RECV* lpMsg, int aIndex) // OK
{
	...
	if (gServerInfo.m_CustomItemViewEquipment[lpObj->AccountLevel] == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(910));
		return;
	}

	if (lpTarget->InfoEquipChar != 1)
	{
...
BYTE CItemManager::MoveItemToPersonalShopFromMuunInventory(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	if(MUUN_INVENTORY_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
	...
	if(this->CheckItemMoveToTrade(lpObj, &lpObj->MuunInventory[SourceSlot], TargetFlag) == 0)
	{
		return 0xFF;
	}

	if(gObjInventorySearchSerialNumber(lpObj, lpObj->MuunInventory[SourceSlot].m_Serial) == 0)
	{
		return 0xFF;
	}

	if(gObjWarehouseSearchSerialNumber(lpObj, lpObj->MuunInventory[SourceSlot].m_Serial) == 0)
	{
		return 0xFF;
	}
...
BYTE CItemManager::MoveItemToMuunInventoryFromPersonalShop(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	#if(GAMESERVER_UPDATE>=603)
	if(INVENTORY_SHOP_RANGE(SourceSlot) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,2,0,"se pasa de rango perro [%d]", SourceSlot);
		return 0xFF;
	}
	//itemLock
	if(lpObj->Lock > 0)
