...
void CMove::Move(LPOBJ lpObj,int index) // OK
{
	...
	if((gGate.GetGateMap(MoveInfo.Gate) == MAP_ICARUS || gGate.GetGateMap(MoveInfo.Gate) == MAP_KANTURU3) && (lpObj->Inventory[7].IsItem() == 0 && lpObj->Inventory[8].m_Index != GET_ITEM(13,3) && lpObj->Inventory[8].m_Index != GET_ITEM(13,37) && lpObj->Inventory[8].m_Index != GET_ITEM(13,4) && gCustomPet.CheckCustomPetIsMontaria(Inv->m_Index) != 1)) // Dinorant,Fenrir
	{
