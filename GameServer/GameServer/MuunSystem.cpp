		...
		while (true)
		{
			...
			while (true)
			{
				if (section == 0)
				{
					...
					info.PetInmortal = lpMemScript->GetAsNumber();
				...
				else if (section == 1)
				{
					...
					info.Option[0] = lpMemScript->GetAsNumber();
					info.Value[0] = lpMemScript->GetAsNumber();

					info.Option[1] = lpMemScript->GetAsNumber();
					info.Value[1] = lpMemScript->GetAsNumber();

					info.Option[2] = lpMemScript->GetAsNumber();
					info.Value[2] = lpMemScript->GetAsNumber();

					info.Option[3] = lpMemScript->GetAsNumber();
					info.Value[3] = lpMemScript->GetAsNumber();

					info.Option[4] = lpMemScript->GetAsNumber();
					info.Value[4] = lpMemScript->GetAsNumber();
...
BYTE CMuunSystem::MuunInventoryInsertItem(int aIndex, CItem item) // OK
{
	...
	for (int y = 0; y < 15; y++)
	{
		for (int x = 0; x < 4; x++)
		{
			...
			{
				BYTE slot = this->MuunInventoryRectCheck(aIndex, x, y, 1, 1);
...
BYTE CMuunSystem::MuunInventoryAddItem(int aIndex, CItem item, int slot) // OK
{
	...
	if (MUUN_INVENTORY_WEAR_RANGE(slot) == 0)
	{
		...
		if (this->MuunInventoryRectCheck(aIndex, x, y, 1, 1) == 0xFF)
		{
			return 0xFF;
		}
...
void CMuunSystem::GetMuunItemSocketOption(int ItemIndex, BYTE* ItemLevel, BYTE* SocketOption, BYTE* SocketOptionBonus) // OK
{
	if (this->IsMuunItem(ItemIndex) != 0)
	{
		...
		if (this->GetMuunSystemInfo(ItemIndex, &MuunSystemInfo) != 0)
		{
...
void CMuunSystem::MuunSprite(LPOBJ lpObj, int damage) // OK
{
	...
	for (int n = 0; n < MUUN_INVENTORY_WEAR_SIZE; n++)
	{
		...
		if (gMuunSystem.GetMuunSystemInfo(lpObj->MuunInventory[n].m_Index, &MuunSystemInfo) != 0)
		{
			if(MuunSystemInfo.PetInmortal != 0 )
			{
				continue;
			}
		}
	...
	for (int n = 0; n < MUUN_INVENTORY_WEAR_SIZE; n++)
	{
		...
		if (this->GetMuunSystemInfo(lpObj->MuunInventory[n].m_Index, &MuunSystemInfo) != 0)
		{
			...
			if (this->GetMuunSystemOptionInfo(MuunSystemInfo.OptionIndex, &MuunSystemOptionInfo) != 0)
			{
				...
				this->InsertOption(lpObj, MuunSystemOptionInfo.Option[0], MuunSystemOptionInfo.Value[0], lpObj->MuunInventory[n].m_Level,flag);
				this->InsertOption(lpObj, MuunSystemOptionInfo.Option[1], MuunSystemOptionInfo.Value[1], lpObj->MuunInventory[n].m_Level,flag);
				this->InsertOption(lpObj, MuunSystemOptionInfo.Option[2], MuunSystemOptionInfo.Value[2], lpObj->MuunInventory[n].m_Level,flag);
				this->InsertOption(lpObj, MuunSystemOptionInfo.Option[3], MuunSystemOptionInfo.Value[3], lpObj->MuunInventory[n].m_Level,flag);
				this->InsertOption(lpObj, MuunSystemOptionInfo.Option[4], MuunSystemOptionInfo.Value[4], lpObj->MuunInventory[n].m_Level,flag);
				//--
				if (this->CheckSpecialOption(lpObj, &MuunSystemOptionInfo) == 0)
				{
					this->InsertOption(lpObj, MuunSystemOptionInfo.SpecialOptionIndex, MuunSystemOptionInfo.SpecialOptionValue, lpObj->MuunInventory[n].m_Level, flag);
				}
			}
		}
	}
...
void CMuunSystem::InsertOption(LPOBJ lpObj, int index, int value, int level,bool flag) // OK
{
#if(GAMESERVER_UPDATE>=603)
	...
	{
	case ADD_HP:
		lpObj->AddLife += value + level * 100;
		break;
	case ADD_MP:
		lpObj->AddMana += value + level * 100;
		break;
	case DMG_RATE:
		lpObj->CriticalDamageRate += value + level;
		lpObj->ExcellentDamageRate += value + level;
		break;
	case ATTK_SPEED:
		...
		break;
	case DOUBLE_DMG:
		lpObj->DoubleDamageRate += value + level;
		break;
	case TRIPLE_DMG:
		lpObj->TripleDamageRate += value + level;
		break;
	case INC_EXP_RATE:
		lpObj->ExperienceRate += value + level;
		lpObj->MasterExperienceRate += value + level;
		break;
	case RES_DOUBLE_DMG:
	...
	case BLOCKSTUCK:
		...
		break;
	case REFLECT_RATE:
		lpObj->DamageReflect += value + level;
		break;
	}
...
void CMuunSystem::CGMuunItemDropRecv(PMSG_MUUN_ITEM_DROP_RECV* lpMsg, int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	PMSG_MUUN_ITEM_DROP_SEND pMsg;

	pMsg.header.set(0x4E, 0x01, sizeof(pMsg));
