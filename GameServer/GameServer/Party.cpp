...
void CParty::CGPartyRequestRecv(PMSG_PARTY_REQUEST_RECV* lpMsg,int aIndex) // OK
{
	...
	int DifLevels = lpObj->Level - lpTarget->Level;

	if (DifLevels > gServerInfo.m_DifferenceMaxLevelParty || DifLevels < -gServerInfo.m_DifferenceMaxLevelParty)
	{
		this->GCPartyResultSend(aIndex, 5);
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(871),gServerInfo.m_DifferenceMaxLevelParty);
		return;
	}
