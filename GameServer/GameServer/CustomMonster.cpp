...
void CCustomMonster::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.RewardIndex1 = lpMemScript->GetAsNumber();

			info.RewardValue1 = lpMemScript->GetAsNumber();

			info.RewardIndex2 = lpMemScript->GetAsNumber();
...
void CCustomMonster::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(CustomMonsterInfo.RewardIndex1 != -1)
	{
		if(CustomMonsterInfo.RewardIndex1 == 0)
		{
			GDCustomMonsterRewardSaveSend(lpTarget->Index,lpObj->Class,lpObj->Map,CustomMonsterInfo.RewardValue1,0);
		}

		if(CustomMonsterInfo.RewardIndex1 == 1)
		{
			GDSetCoinSend(lpTarget->Index, CustomMonsterInfo.RewardValue1, 0, 0,"Custom Monster R1 - WCoinC");
		}

		if(CustomMonsterInfo.RewardIndex1 == 2)
		{
			GDSetCoinSend(lpTarget->Index, 0, CustomMonsterInfo.RewardValue1, 0,"Custom Monster R1 - WCoinP");
		}

		if(CustomMonsterInfo.RewardIndex1 == 3)
		{
			GDSetCoinSend(lpTarget->Index, 0, 0, CustomMonsterInfo.RewardValue1,"Custom Monster R1 - GoblinPoint");
		}
	}

	if(CustomMonsterInfo.RewardIndex2 != -1)
	{
		if(CustomMonsterInfo.RewardIndex2 == 0)
		{
			GDCustomMonsterRewardSaveSend(lpTarget->Index,lpObj->Class,lpObj->Map,0,CustomMonsterInfo.RewardValue2);
		}

		if(CustomMonsterInfo.RewardIndex2 == 1)
		{
			GDSetCoinSend(lpTarget->Index, CustomMonsterInfo.RewardValue2, 0, 0,"Custom Monster R1 - WCoinC");
		}

		if(CustomMonsterInfo.RewardIndex2 == 2)
		{
			GDSetCoinSend(lpTarget->Index, 0, CustomMonsterInfo.RewardValue2, 0,"Custom Monster R1 - WCoinP");
		}

		if(CustomMonsterInfo.RewardIndex2 == 3)
		{
			GDSetCoinSend(lpTarget->Index, 0, 0, CustomMonsterInfo.RewardValue2,"Custom Monster R1 - GoblinPoint");
		}
	}
