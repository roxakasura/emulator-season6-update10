		...
		if(gServerInfo.m_CashShopCoinsDelay >= 1)
		{
         if((GetTickCount()-lpObj->CashShopGoblinPointTime) >= ((DWORD)gServerInfo.m_CashShopCoinsDelay*60000))
            ...
            this->CGCashShopPointRecv(lpObj->Index);
					...
					{
					gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(862),gServerInfo.m_CashShopWCoinCValue[lpObj->AccountLevel]);
					}

					if(gServerInfo.m_CashShopWCoinPValue[lpObj->AccountLevel] >= 1)
					{
					gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(863),gServerInfo.m_CashShopWCoinPValue[lpObj->AccountLevel]);
					}
					
					if(gServerInfo.m_CashShopGoblinPointValue[lpObj->AccountLevel] >= 1)
					{
					gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(864),gServerInfo.m_CashShopGoblinPointValue[lpObj->AccountLevel]);
					}
			}
		}
