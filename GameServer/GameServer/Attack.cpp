...
bool CAttack::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int damage,int count,bool combo) // OK
{
	...
	if(damage == 0)
	{
		...
		this->ResetSprite(lpObj,lpTarget,&damage);
...
void CAttack::ResetSprite(LPOBJ lpObj,LPOBJ lpTarget,int* damage)
{
	if(gServerInfo.m_DamageResetSwitch == 0)
	{
		return;
	}

	if(lpObj != 0 && lpObj->Type == OBJECT_USER 
		&& lpTarget != 0 && lpTarget->Type == OBJECT_USER)
	{
		if(lpObj->MasterReset == lpTarget->MasterReset)
		{
			return;
		}
		...
		if(lpObj->MasterReset > lpTarget->MasterReset)
		{
			(*damage) = ((*damage)*(100 + (lpObj->MasterReset - lpTarget->MasterReset) * gServerInfo.m_DamageResetPercent)) / 100;
		}
		else
		{
			(*damage) = ((*damage)*(100 - (lpTarget->MasterReset - lpObj->MasterReset) * gServerInfo.m_DefenseResetPercent)) / 100;
		}
	}
}
...
void CAttack::DamageSprite(LPOBJ lpObj,int damage) // OK
{
	...
	/*							Angel (13,0)							*/
	float DurabilityAngelValue = (1.0f/gServerInfo.m_PetAngelDurabilityRate)*100;
	DurabilityAngelValue = (DurabilityAngelValue/lpObj->GuardianDurabilityRate)*100;
	/*																	*/
	/*							Satan (13,1)							*/
	float DurabilitySatanValue = (1.0f/gServerInfo.m_PetSatanDurabilityRate)*100;
	DurabilitySatanValue = (DurabilitySatanValue/lpObj->GuardianDurabilityRate)*100;
	/*																	*/
	/*							Uniria (13,2)							*/
	float DurabilityUniriaValue = (1.0f/gServerInfo.m_PetUniriaDurabilityRate)*100;
	DurabilityUniriaValue = (DurabilityUniriaValue/lpObj->GuardianDurabilityRate)*100;
	/*																	*/
	/*							Dinorant (13,3)							*/
	float DurabilityDinorantValue = (1.0f/gServerInfo.m_PetDinorantDurabilityRate)*100;
	DurabilityDinorantValue = (DurabilityDinorantValue/lpObj->GuardianDurabilityRate)*100;
	/*																		*/
	/*							Demon (13,64)							*/
	float DurabilityDemonValue = (1.0f/gServerInfo.m_PetDemonDurabilityRate)*100;
	DurabilityDemonValue = (DurabilityDemonValue/lpObj->GuardianDurabilityRate)*100;
	/*																		*/
	/*							Maria (13,65)							*/
	float DurabilityMariaValue = (1.0f/gServerInfo.m_PetMariaDurabilityRate)*100;
	DurabilityMariaValue = (DurabilityMariaValue/lpObj->GuardianDurabilityRate)*100;
	/*																		*/
	/*							Rudolf (13,67)							*/
	float DurabilityRudolfValue = (1.0f/gServerInfo.m_PetRudolfDurabilityRate)*100;
	DurabilityRudolfValue = (DurabilityRudolfValue/lpObj->GuardianDurabilityRate)*100;
	/*																		*/
	/*							Panda (13,80)							*/
	float DurabilityPandaValue = (1.0f/gServerInfo.m_PetPandaDurabilityRate)*100;
	DurabilityPandaValue = (DurabilityPandaValue/lpObj->GuardianDurabilityRate)*100;
	/*																		*/
	/*							Unicorn (13,106)							*/
	float DurabilityUnicornValue = (1.0f/gServerInfo.m_PetUnicornDurabilityRate)*100;
	DurabilityUnicornValue = (DurabilityUnicornValue/lpObj->GuardianDurabilityRate)*100;
	/*																		*/
	/*							Skeleton (13,123)							*/
	float DurabilitySkeletonValue = (1.0f/gServerInfo.m_PetSkeletonDurabilityRate)*100;
	DurabilitySkeletonValue = (DurabilitySkeletonValue/lpObj->GuardianDurabilityRate)*100;
	/*																		*/
	/*								CustomPet								*/
	int m_PetSkeletonDurabilityRate = gCustomPet.GetCustomPetIsImortal(lpItem->m_Index);
	float DurabilityCustomPetValue = (1.0f/gCustomPet.m_CustomPetInfo->IsImortal)*100;
	DurabilityCustomPetValue = (DurabilityCustomPetValue/lpObj->GuardianDurabilityRate)*100;


	if(lpItem->m_Index == GET_ITEM(13,0)) // Angel
	{
		if(gServerInfo.m_PetAngelDurabilityRate == 0)
		{
			lpItem->m_Durability = lpItem->m_Durability;
		}
		else
		{
			lpItem->m_Durability -= (damage*(3.0f*DurabilityAngelValue))/100;	/*			Reformulado			*/
		}
	}
	else if(lpItem->m_Index == GET_ITEM(13,1)) // Satan
	{
		if(gServerInfo.m_PetSatanDurabilityRate == 0)
		{
			lpItem->m_Durability = lpItem->m_Durability;
		}
		else
		{
			lpItem->m_Durability -= (damage*(2.0f*DurabilitySatanValue))/100;	/*			Reformulado			*/
		}
	}
	else if(lpItem->m_Index == GET_ITEM(13,2)) // Uniria
	{
		if(gServerInfo.m_PetUniriaDurabilityRate == 0)
		{
			lpItem->m_Durability = lpItem->m_Durability;
		}
		else
		{
			lpItem->m_Durability -= (damage*(1.0f*DurabilityUniriaValue))/100;	/*			Reformulado			*/
		}
	}
	else if(lpItem->m_Index == GET_ITEM(13,3)) // Dinorant
	{
		if(gServerInfo.m_PetDinorantDurabilityRate == 0)
		{
			lpItem->m_Durability = lpItem->m_Durability;
		}
		else
		{
			lpItem->m_Durability -= (damage*(1.0f*DurabilityDinorantValue))/100;	/*			Reformulado			*/
		}
	}
	else if(lpItem->m_Index == GET_ITEM(13,4)) // Dark Horse
	{
	...
	{
		if(gServerInfo.m_PetDemonDurabilityRate == 0)
		{
			lpItem->m_Durability = lpItem->m_Durability;
		}
		else
		{
			lpItem->m_Durability -= (damage*(1.0f*DurabilityDemonValue))/100;	/*			Reformulado			*/
		}
	}
	else if(lpItem->m_Index == GET_ITEM(13,65)) // Maria
	{
		if(gServerInfo.m_PetMariaDurabilityRate == 0)
		{
			lpItem->m_Durability = lpItem->m_Durability;
		}
		else
		{
			lpItem->m_Durability -= (damage*(2.0f*DurabilityMariaValue))/100;	/*			Reformulado			*/
		}
	}
	else if(lpItem->m_Index == GET_ITEM(13,67)) // Rudolf
	{
		if(gServerInfo.m_PetRudolfDurabilityRate == 0)
		{
			lpItem->m_Durability = lpItem->m_Durability;
		}
		else
		{
			lpItem->m_Durability -= (damage*(1.0f*DurabilityRudolfValue))/100;	/*			Reformulado			*/
		}
	}
	else if(lpItem->m_Index == GET_ITEM(13,80)) // Panda
	{
		if(gServerInfo.m_PetPandaDurabilityRate == 0)
		{
			lpItem->m_Durability = lpItem->m_Durability;
		}
		else
		{
			lpItem->m_Durability -= (damage*(1.0f*DurabilityPandaValue))/100;	/*			Reformulado			*/
		}
	}
	else if(lpItem->m_Index == GET_ITEM(13,106)) // Unicorn
	{
		if(gServerInfo.m_PetUnicornDurabilityRate == 0)
		{
			lpItem->m_Durability = lpItem->m_Durability;
		}
		else
		{
			lpItem->m_Durability -= (damage*(1.0f*DurabilityUnicornValue))/100;	/*			Reformulado			*/
		}
	}
	else if(lpItem->m_Index == GET_ITEM(13,123)) // Skeleton
	{
		if(gServerInfo.m_PetSkeletonDurabilityRate == 0)
		{
			lpItem->m_Durability = lpItem->m_Durability;
		}
		else
		{
			lpItem->m_Durability -= (damage*(1.0f*DurabilitySkeletonValue))/100;	/*			Reformulado			*/
		}
	}
	else if(gCustomPet.CheckCustomPetByItem(lpItem->m_Index) != 0) // CustomPets
	{
		if(gCustomPet.CheckCustomPetIsImortal(lpItem->m_Index) == 0)
		{
			lpItem->m_Durability = lpItem->m_Durability;
		}
		else
		{
			lpItem->m_Durability -= (damage*(3.0f*DurabilityCustomPetValue))/100;
		}
	}
	else
	{
	...
	if(lpItem->m_Durability < 1)
	{
		...
		CUSTOM_PET_INFO* isMont = gCustomPet.GetInfoByItem(Inv->m_Index);    //<- aqu� custommonsterskill

		if(lpObj->Map == MAP_ICARUS && lpObj->Inventory[7].IsItem() == 0 && (lpItem->m_Index == GET_ITEM(13,3) || lpItem->m_Index == GET_ITEM(13,37) || lpObj->Inventory[8].m_Index == GET_ITEM(13,4) || isMont->IsMontaria == 1))
		{
...
bool CAttack::DarkHorseSprite(LPOBJ lpObj,int damage) // OK
{
	...
	int MaxSmallDur = (200*gServerInfo.m_PetHorseDurabilityRate)/100;

	MaxSmallDur = (MaxSmallDur*lpObj->PetDurabilityRate)/100;

	if(lpItem->m_DurabilitySmall > MaxSmallDur)
	{
		if(gServerInfo.m_PetRavenDurabilityRate == 0)
		{
			lpItem->m_Durability = lpItem->m_Durability;
		}
		else
...
bool CAttack::FenrirSprite(LPOBJ lpObj,int damage) // OK
{
	...
	int MaxSmallDur = (200*gServerInfo.m_PetFenrirDurabilityRate)/100;

	MaxSmallDur = (MaxSmallDur*lpObj->GuardianDurabilityRate)/100;

	if(lpItem->m_DurabilitySmall > MaxSmallDur)
	{
		if(gServerInfo.m_PetFenrirDurabilityRate == 0)
		{
			lpItem->m_Durability = lpItem->m_Durability;
		}
		else
...
bool CAttack::CheckPlayerTarget(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(gPvPCoord.CheckPvPCoord(lpTarget->Map,lpTarget->X,lpTarget->Y, 0))
	{
		return 0;
	}

	if(gPvPCoord.CheckPvPCoord(lpTarget->Map,lpTarget->X,lpTarget->Y, 1) || gPvPCoord.CheckPvPCoord(lpTarget->Map,lpTarget->X,lpTarget->Y, 2))
	{
		return 1;
	}
...
{
	int DisableSD				= GetPrivateProfileInt("GameServerInfo","DisableSD",0,".\\Data\\GameServerInfo - Skill.dat");
