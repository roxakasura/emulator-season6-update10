...
enum eCommandNumber
{
	...
	COMMAND_ADDPK			= 83,
	COMMAND_STARTLUCKYWHEEL = 84,
	COMMAND_SETLEVEL		= 85,
	COMMAND_BLOCCHAT		= 86,
	COMMAND_CLEARVAULT		= 87,
};
...
struct SDHP_COMMAND_OFFPVP_RECV
{
...
struct SDHP_COMMAND_OFFPVP_SEND
{
	PSBMSG_HEAD header; // C1:0F:00
	WORD index;
	char account[11];
...
public:
	...
	bool CommandAddPK(LPOBJ lpObj,char* arg, int Npc); // OK
	...
	//Teste
	void CommandDisablePvPProc(LPOBJ lpObj);
	//Teste
	...
	bool CommandLuckyWheel(LPOBJ lpObj, char* arg);
	bool CommandSetLevel(LPOBJ lpObj, char* arg);
	bool CommandBlocChat(LPOBJ lpObj, char* arg);
	bool CommandTest(LPOBJ lpObj, char* arg);
	int m_Active;
private:
