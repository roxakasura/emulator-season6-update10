...
void gObjCharZeroSet(int aIndex) // OK
{
	...
	gBankEx.InitUser(lpObj); //Jewel Bank V1
	JCBankJewelry.InitUserBank(lpObj); //Jewel Bank V2
...
void gObjUserDie(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	//Drop item system
	if(gServerInfo.m_PkItemDropSwitch == 1 && gServerInfo.m_PkItemDropEnable[lpObj->AccountLevel] == 1 && gServerInfo.m_PkItemDropRate >= rand()%100 && gMapManager.GetMapPkDropItem(lpObj->Map) == 1) //add map manager
	{
		if(lpObj->PKLevel >= 6)
		{
			count = 24;
...
//pk free
	if(!gPvPCoord.CheckPvPCoord(lpObj->Map,lpObj->X,lpObj->Y, 2)){
...
BOOL gObjMoveGate(int aIndex,int gate) // OK
{
	...
	CUSTOM_PET_INFO* isMont = gCustomPet.GetInfoByItem(Inv->m_Index);    //<- aquí custommonsterskill

	if((map == MAP_ICARUS || map == MAP_KANTURU3) && lpObj->Inventory[7].IsItem() == 0 && lpObj->Inventory[8].m_Index != GET_ITEM(13,3) && lpObj->Inventory[8].m_Index != GET_ITEM(13,37) && lpObj->Inventory[8].m_Index != GET_ITEM(13,4) && isMont->IsMontaria != 1)
	{
...
void MontariaRelocadaProc()
{
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		...
		//GCSetMountSafeZoneSend(lpObj);
		LogAdd(LOG_EVENT, "valor2: %d", gCommandManager.m_Active);
	}
}
