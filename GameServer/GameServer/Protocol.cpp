...
void ProtocolCore(BYTE head,BYTE* lpMsg,int size,int aIndex,int encrypt,int serial) // OK
{
	...
	switch(head)
	{
		...
		case 0x19:
			...
			FixSkillsNoWeapor(lpMsg, aIndex);
			break;
		case 0x1B:
		...
		case 0x4E:
			switch(lpMsg[3])
			{
				case 0x00:
					...
					break;
				case 0x01:
					gMuunSystem.CGMuunItemDropRecv((PMSG_MUUN_ITEM_DROP_RECV*)lpMsg,aIndex);
					break;
				case 0x08:
		...
		case 0xF3:
			switch(lpMsg[3])
			{
				...
				case 0xF0:
					...
					break;
				case 0xF1:
					JCBankJewelry.RecvSubPacket(aIndex,(PMSG_JEWELBANK_SUB*)lpMsg);
					break;
				case 0xF2:
					GCItemEquippedStatus(aIndex, (PMSG_ITEMVIEW_RECV*)lpMsg);
					break;
				case 0xDF:
					JCBankJewelry.RecvPacket(aIndex, (PMSG_JEWEL_BANK*)lpMsg);
					break;
				case 0xF4:
		...
		case 0xFB:
			...
			switch(lpMsg[3])
			{
				...
				case 0x3D:
					...
					break;
				case 0xDA:
					gBankEx.Packet(aIndex, (PMSG_JEWELBANK*)lpMsg);
					break;
			}
			break;
		case 0xFC:
...
void CGChatRecv(PMSG_CHAT_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpObj->BlockChat == 1) //Comando Bloquear Mensagens
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(888));
		return;
	}
	...
	if(lpMsg->message[0] == '/')
	{
		...
		else if(gCustomExchangeCoin.GetInfoByName(lpObj,lpMsg->message,-1) != 0)
		{
			return;
		}
		else
...
void FixSkillsNoWeapor(LPBYTE lpMsg, int aIndex)
{
    LPOBJ lpObj = &gObj[aIndex];
	...
	for(int n=0;n < INVENTORY_WEAR_SIZE;n++)
	{
		if(lpObj->Inventory[n].m_Option1 != 0)
			{
				if(lpObj->Class == 16 || lpObj->Class == 17 || lpObj->Class == 19 || lpObj->Class == 64 || lpObj->Class == 66)
				{
					lpMsg[4] = 0xFF;
				}
				if(lpObj->Class == 64 || lpObj->Class == 66)
				{            
					lpMsg[4] = 0xFF;            
				}
			}
...
void GCSetItemViewSend(LPOBJ lpObj)
{
	PMSG_ITEMVIEW_SEND pMsg;

	pMsg.header.set(0xFB, 0xD4, sizeof(pMsg));

	pMsg.Status = lpObj->InfoEquipChar;

	DataSend(lpObj->Index, (BYTE*)&pMsg, pMsg.header.size);
}

void GCItemEquippedStatus(int aIndex, PMSG_ITEMVIEW_RECV* lpMsg)
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	if (gServerInfo.m_CustomItemViewEquipment[lpObj->AccountLevel] == 1)
	{
		if (lpMsg->Status == 0)
		{
			lpObj->InfoEquipChar = 0;
			GDSaveEquipmentView(lpObj->Index);
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(907));
		}
		...
		if (lpMsg->Status == 1)
		{
			...
			GDSaveEquipmentView(lpObj->Index);
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(908));
		}
	}
