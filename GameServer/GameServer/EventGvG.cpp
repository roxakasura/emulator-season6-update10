...
void CGvGEvent::ProcState_EMPTY() // OK
{
	if(this->m_RemainTime > 0 && this->m_RemainTime <= (this->m_WarningTime*60))
	{
		...
		if((this->AlarmMinSave=(((this->m_RemainTime%60)==0)?((this->m_RemainTime/60)-1):(this->m_RemainTime/60))) != this->AlarmMinLeft)
		{
			...
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(889),(this->AlarmMinLeft+1));
		}
	}

	if(this->m_RemainTime <= 0)
	{
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(891));
		this->NoticeSendToAll(1,gMessage.GetMessage(892),this->m_StandTime);
...
void CGvGEvent::ProcState_STAND() // OK
{
	...
	{
		this->NoticeSendToAll(0,gMessage.GetMessage(901));
		this->SetState(GVG_EVENT_STATE_EMPTY);
		return;
	}
	...
	if(this->m_RemainTime <= 0)
	{
		if (this->TotalPlayer >= gServerInfo.m_GvGEventMinUsers)
		{
			...
			this->NoticeSendToAll(0,gMessage.GetMessage(893));
			this->SetState(GVG_EVENT_STATE_START);
		}
		else
		{
			this->NoticeSendToAll(0,gMessage.GetMessage(894));
			this->SetState(GVG_EVENT_STATE_EMPTY);
		}
	}
}
...
bool CGvGEvent::Dialog(LPOBJ lpObj, LPOBJ lpNpc)
{
	...
	{
		GCChatTargetSend(lpObj, lpNpc->Index, gMessage.GetMessage(890));
...
bool CGvGEvent::CheckEnterEnabled(LPOBJ lpObj)
{
	...
	if(lpObj->Guild == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(898));
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(896));
		return 0;
	}

	if (lpObj->GuildStatus == G_MASTER && this->CheckReqItems(lpObj) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(897));
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(899),this->MaxMasterReset);
		return 0;
	}

	if (lpObj->GuildStatus == G_MASTER && this->AddGuild(lpObj->GuildNumber) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(895),this->MaxMasterReset);
		return 0;
	}
...
void CGvGEvent::UserDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	this->NoticeSendToAll(0,gMessage.GetMessage(900),lpGuild->Point,lpTarget->GuildName);
}
	...
	for(int n=0;n < MAX_GVGEVENT_GUILD;n++)
	{
		if (this->Guild[n].Guild >= 0 )
		{
			...
			if (this->Guild[n].Rank == 1) 
			{
				...
				if (lpGuildInfo != NULL)
				{
					this->NoticeSendToAll(0,gMessage.GetMessage(902),lpGuildInfo->Name);
				}
