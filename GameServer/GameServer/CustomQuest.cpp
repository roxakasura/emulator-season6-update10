...
bool CCustomQuest::CheckCharacter(LPOBJ lpObj,int quest,int Npc) // OK
{
	for(std::vector<CUSTOM_QUEST_CHARACTER>::iterator it=this->m_CustomQuestCharacterInfo.begin();it != this->m_CustomQuestCharacterInfo.end();it++)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(692), it->Level);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(692), it->Level);
			}
			return 0;
		}
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(693), it->Reset);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(693), it->Reset);
			}
			return 0;
		}
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(694), it->MReset);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(694), it->MReset);
			}
			return 0;
		}
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(695), it->Zen);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(695), it->Zen);
			}
			return 0;
		}
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(849), it->Coin1);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(849), it->Coin1);
			}
			return 0;
		}
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(850), it->Coin2);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(850), it->Coin2);
			}
			return 0;
		}
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(851), it->Coin3);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(851), it->Coin3);
			}
			return 0;
		}
		...
		if (it->VipLevel >= 0 && lpObj->AccountLevel < it->VipLevel)
		{
			...
			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(697));
			}
			return 0;
		}
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(856));

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(856));
			}
			return 0;
		}
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(857), it->Kills);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(857), it->Kills);
			}
			return 0;
		}
	}
	...
	for(std::vector<CUSTOM_QUEST_ITEM>::iterator it=this->m_CustomQuestItemInfo.begin();it != this->m_CustomQuestItemInfo.end();it++)
	{
		...
		if (it->Category >= 0 && it->ItemIndex >= 0)
		{
			...
			if (Qtd < it->Quantity)
			{
				...
				gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(698),text);

				if (Npc >= 0)
				{
					GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(698),text);
				}
				return 0;
			}
		}
...
bool CCustomQuest::CheckMonster(LPOBJ lpObj,int quest,int Npc) // OK
{
	for(std::vector<CUSTOM_QUEST_MONSTER>::iterator it=this->m_CustomQuestMonsterInfo.begin();it != this->m_CustomQuestMonsterInfo.end();it++)
	{
		...
		if (it->MonsterIndex >= 0 && lpObj->CustomQuestMonsterQtd != 0)
		{
			...
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(699), it->Quantity, gMonsterManager.GetMonsterName(it->MonsterIndex),lpObj->CustomQuestMonsterQtd);

			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(699), it->Quantity, gMonsterManager.GetMonsterName(it->MonsterIndex),lpObj->CustomQuestMonsterQtd);
			}
			return 0;
		}
