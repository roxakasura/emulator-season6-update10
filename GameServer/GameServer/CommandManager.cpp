...
void CCommandManager::MainProc() // OK
{
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		...
		if(gObj[n].DisablePvp != 0)
		{
			this->CommandDisablePvPProc(&gObj[n]);
		}
	}
}
...
bool CCommandManager::ManagementCore(LPOBJ lpObj,char* message, int Npc) // OK
{
	...
	switch(code)
	{
		...
		case COMMAND_STARTARENA:
			...
			break;
		case COMMAND_STARTLUCKYWHEEL:
			Result = CommandLuckyWheel(lpObj, argument);
			break;
		case COMMAND_SETLEVEL:
			Result = CommandSetLevel(lpObj, argument);
			break;
		case COMMAND_BLOCCHAT:
			Result = CommandBlocChat(lpObj, argument);
			break;
		default:
			return 0;
	}
...
bool CCommandManager::CommandAddPK(LPOBJ lpObj,char* arg, int Npc) // OK
{
	...
	if (Npc >= 0)
	{
	GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(873));
	}
...
bool CCommandManager::CommandMasterReset(LPOBJ lpObj,char* arg,int Npc) // OK
{
	...
	gCashShop.GDCashShopAddPointSaveSend(lpObj->Index,0,gBonusManager.GetBonusValue(lpObj,BONUS_INDEX_MASTER_RESET_AMOUNT,gServerInfo.m_CommandMasterWCoinCValue[lpObj->AccountLevel],-1,-1,-1,-1),
                                                gBonusManager.GetBonusValue(lpObj,BONUS_INDEX_MASTER_RESET_AMOUNT,gServerInfo.m_CommandMasterWCoinPValue[lpObj->AccountLevel],-1,-1,-1,-1),
                                                gBonusManager.GetBonusValue(lpObj,BONUS_INDEX_MASTER_RESET_AMOUNT,gServerInfo.m_CommandMasterGoblinPointValue[lpObj->AccountLevel],-1,-1,-1,-1));
	gCashShop.CGCashShopPointRecv(lpObj->Index);

	if(gServerInfo.m_CommandMasterEventItemBagSwitch[lpObj->AccountLevel] == 1)
	{
		if(gServerInfo.m_CommandMasterResetMove[lpObj->AccountLevel] != 0)
		{
			...
			{
				case CLASS_DW:
				gItemBagManager.DropItemBySpecialValue(ITEM_BAG_MRESET,lpObj,0,125,125); 
				break;
				...
				case CLASS_FE:
				gItemBagManager.DropItemBySpecialValue(ITEM_BAG_MRESET,lpObj,3,125,125); 
				break;
		...
		{
				gItemBagManager.DropItemBySpecialValue(ITEM_BAG_MRESET,lpObj,lpObj->Map,lpObj->X,lpObj->Y); 
...
//TESTE
void CCommandManager::CommandDisablePvPProc(LPOBJ lpObj) // OK
{
	if(lpObj->Interface.use != 0 || lpObj->State == OBJECT_DELCMD || lpObj->DieRegen != 0 || lpObj->Teleport != 0 || lpObj->PShopOpen != 0 || lpObj->SkillSummonPartyTime != 0)
	{
		return;
	...
	if (lpObj->Level > gServerInfo.m_CustomOffPvpLevelDisable[lpObj->AccountLevel]) //Disable when vip over
	{
		lpObj->DisablePvp = 0; // Sets AutoResetEnable para OFF
		gCustomAttack.DGCustomAttackResumeSaveSend(lpObj->Index);
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(772)); // Escolher a MSG
		return;
	}

	SDHP_COMMAND_OFFPVP_SEND pMsg;

	pMsg.header.set(0x0F,0x0A,sizeof(pMsg));
...
void CCommandManager::DGCommandResetRecv(SDHP_COMMAND_RESET_RECV* lpMsg) // OK
{
	...
	gCashShop.GDCashShopAddPointSaveSend(lpObj->Index,0,gBonusManager.GetBonusValue(lpObj,BONUS_INDEX_RESET_AMOUNT,gServerInfo.m_CommandResetWCoinCValue[lpObj->AccountLevel],-1,-1,-1,-1),
                                                gBonusManager.GetBonusValue(lpObj,BONUS_INDEX_RESET_AMOUNT,gServerInfo.m_CommandResetWCoinPValue[lpObj->AccountLevel],-1,-1,-1,-1),
                                                gBonusManager.GetBonusValue(lpObj,BONUS_INDEX_RESET_AMOUNT,gServerInfo.m_CommandResetGoblinPointValue[lpObj->AccountLevel],-1,-1,-1,-1));
	gCashShop.CGCashShopPointRecv(lpObj->Index);

	gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(95),lpObj->Reset);

	if(gServerInfo.m_CommandResetWCoinCValue[lpObj->AccountLevel] >= 1)
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(868),gServerInfo.m_CommandResetWCoinCValue[lpObj->AccountLevel]);
		}

	if(gServerInfo.m_CommandResetWCoinPValue[lpObj->AccountLevel] >= 1)
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(869),gServerInfo.m_CommandResetWCoinPValue[lpObj->AccountLevel]);
		}
					
	if(gServerInfo.m_CommandResetGoblinPointValue[lpObj->AccountLevel] >= 1)
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(870),gServerInfo.m_CommandResetGoblinPointValue[lpObj->AccountLevel]);
		}
...
void CCommandManager::DGCommandMasterResetRecv(SDHP_COMMAND_MASTER_RESET_RECV* lpMsg) // OK
{
	...
	if(gServerInfo.m_CommandMasterWCoinCValue[lpObj->AccountLevel] >= 1)
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(865),gServerInfo.m_CommandMasterWCoinCValue[lpObj->AccountLevel]);
		}

	if(gServerInfo.m_CommandMasterWCoinPValue[lpObj->AccountLevel] >= 1)
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(866),gServerInfo.m_CommandMasterWCoinPValue[lpObj->AccountLevel]);
		}
					
	if(gServerInfo.m_CommandMasterGoblinPointValue[lpObj->AccountLevel] >= 1)
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(867),gServerInfo.m_CommandMasterGoblinPointValue[lpObj->AccountLevel]);
		}
...
bool CCommandManager::CommandLuckyWheel(LPOBJ lpObj, char* arg) // OK
{
	gLuckyWheel.StartLucky();
	gLog.Output(LOG_COMMAND, "[CommandStartLuckyWheel][%s][%s] - Used", lpObj->Account, lpObj->Name);
	return 1;
}
...
bool CCommandManager::CommandInfo(LPOBJ lpObj, char* arg) // OK
{
	...
	pMsg.Level = lpTarget->Level;

	pMsg.Resets = lpTarget->Reset;
...
bool CCommandManager::CommandBlocChat(LPOBJ lpObj, char* arg) // OK
{
	char name[11] = { 0 };
	int Teste;
	...
	Teste = this->GetNumber(arg, 1);

	if (Teste == 0) //Desbloqueia o ChatSystem
	{
		lpTarget->BlockChat = 0;
		GDSaveBlocChatData(lpTarget->Index);
		gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(886));
	}

	if (Teste == 1) //Bloqueia o ChatSystem
	{
		lpTarget->BlockChat = 1;
		GDSaveBlocChatData(lpTarget->Index);
		gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(887));
	}

	gLog.Output(LOG_COMMAND, "[CommandBlocChat][%s][%s] - (Name: %s)", lpObj->Account, lpObj->Name, name);

	return 1;
}

bool CCommandManager::CommandTest(LPOBJ lpObj, char* arg) // OK
{
	int Teste;

	Teste = this->GetNumber(arg, 0);

	if (Teste == 0) //Desbloqueia o ChatSystem
	{
		lpObj->InfoEquipChar = 0;
		GDSaveEquipmentView(lpObj->Index);
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "message type 0");
	}

	if (Teste == 1) //Bloqueia o ChatSystem
	{
		lpObj->InfoEquipChar = 1;
		GDSaveEquipmentView(lpObj->Index);
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "message type 1");
	}

	gLog.Output(LOG_COMMAND, "[CommandTest] Use for:[%s]", lpObj->Account);
	return 1;
