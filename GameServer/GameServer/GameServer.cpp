...
int APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow) // OK
{
	...
	wsprintf(buff,"[%s] %s (Online: %d) - RoxGaming.com.br",GAMESERVER_VERSION,gServerInfo.m_ServerName, gObjTotalUser);
...
BOOL InitInstance(HINSTANCE hInstance,int nCmdShow) // OK
{
	...
	hWnd = CreateWindowA(szWindowClass, szTitle, WS_OVERLAPPED + WS_CAPTION + WS_SYSMENU + WS_MINIMIZEBOX, CW_USEDEFAULT, 0, 600,600, NULL, NULL, hInstance, NULL); // TAMANHO DO GS 
...
LRESULT CALLBACK WndProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam) // OK
{
	...
	switch(message)
	{
...

        {
			...
			wsprintf(text, "Update %d", (GSUPDATE+1));
