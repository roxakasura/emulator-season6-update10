...
bool CCustomExchangeCoin::GetInfoByName(LPOBJ lpObj, char* message, int Npc) // OK
{
	...
	for(std::map<int,CUSTOMEXCHANGECOIN_INFO>::iterator it=this->m_CustomExchangeCoinInfo.begin();it != this->m_CustomExchangeCoinInfo.end();it++)
	{
		if(_stricmp(it->second.Name,command) == 0)
		{
			...
			{
				gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(903), it->second.RequireCoin1);
				if (Npc >= 0)
				{
					GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(903), it->second.RequireCoin1);
				}
				return 1;
			}
			if (it->second.RequireCoin2 > 0 && it->second.RequireCoin2 > lpObj->Coin2)
			{
				gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(904), it->second.RequireCoin2);
				if (Npc >= 0)
				{
					GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(904), it->second.RequireCoin2);
				}
				return 1;
			}
			if (it->second.RequireCoin3 > 0 && it->second.RequireCoin3 > lpObj->Coin3)
			{
				gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(905), it->second.RequireCoin3);
				if (Npc >= 0)
				{
					GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(905), it->second.RequireCoin3);
				}
				return 1;
			}

			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(906));
			if (Npc >= 0)
			{
				GCChatTargetNewSend(lpObj,Npc,gMessage.GetMessage(906));
			}
