...
void InitPrintPlayer() // OK
{
	...
	//SetCompleteHook(0xE9,0x077FAE0,&sub_77FAE0);
	...
	//SetCompleteHook(0xE8,0x0080FF28,&DrawHp);
	//
	//SetCompleteHook(0xE8,0x008100BB,&DrawMp);
...
void PrintDamageOnScreenHP(DWORD a,DWORD b,DWORD c,DWORD d,DWORD e,DWORD f) // OK
{
	if(((int)b) > 0)
	{
		if(gProtect.m_MainInfo.m_StatusRealView == 0)
		{
			if(ViewDamageHP > 65000)
			{
				ViewDamageHP = 65000;
			}
		}
...
int PrintPlayerLevel3(char * a1, char * a2, ...)
{	
	char Nome[200];
	wsprintf(Nome,"Denis");
	return sub_50D100(Nome,"Denis");
}
...
void DrawHp(float a1, float a2, int Value, float a4)
{
   ...
   pDrawBar(/*155.0f*/a1, 445.0f, ViewCurHP, 1.0f);
}
