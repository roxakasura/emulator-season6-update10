...
struct PMSG_ITEMVIEW_RECV
{
	PSBMSG_HEAD	header;
	int Status;
};

struct PMSG_LUCKYWHEELACTIVE_RECV
{
	PSBMSG_HEAD	header;
	int Active;
};
...
void UpdateLuckyWheel(ITEM_WIN_SEND * lpMsg); //luckywheel
bool CashShopSwitchState();
void GCSetCharacterPage(PMSG_CHARACTERPAGE_RECV* lpMsg); //multi selectchar
void CGSendMultiChar(int Page); //multi selectchar
